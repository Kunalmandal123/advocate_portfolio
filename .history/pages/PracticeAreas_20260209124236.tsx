...
const PracticeAreas: React.FC = () => {
  ...
  // Create a copy and swap only Criminal Defense ↔ Property Matters
  const swappedPracticeAreas = [...PRACTICE_AREAS];

  const criminalIndex = swappedPracticeAreas.findIndex(area => area.id === 'criminal-law');
  const propertyIndex = swappedPracticeAreas.findIndex(area => area.id === 'property-disputes');

  if (criminalIndex !== -1 && propertyIndex !== -1) {
    // Swap the two items
    [swappedPracticeAreas[criminalIndex], swappedPracticeAreas[propertyIndex]] = 
    [swappedPracticeAreas[propertyIndex], swappedPracticeAreas[criminalIndex]];
  }
